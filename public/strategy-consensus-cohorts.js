...
function stats(feature) {
  if (Number.isFinite(Number(feature?.strategyMatchCount)) && Number.isFinite(Number(feature?.strategyAxisCount))) {
    return feature;
  }
  return { ...feature, ...evaluateBaseConsensus(feature) };
}
...
export const CONSENSUS_STRATEGIES = [
  ...
  condition({
    ...
    selector: (f) => {
      const s = stats(f);
      return s.strategyMatchCount >= 5 && s.strategyAxisCount >= 3;
    },
  ...
  condition({
    ...
    selector: (f) => {
      const s = stats(f);
      return s.strategyMatchCount >= 5 && s.strategyAxisCount >= 4;
    },
  ...
  condition({
    ...
    selector: (f) => {
      const s = stats(f);
      return s.strategyAxisCount >= 3 && hasLeaderRs(s);
    },
  ...
  condition({
    ...
    selector: (f) => {
      const s = stats(f);
      return s.strategyAxisCount >= 4 && hasLeaderRs(s);
    },
  ...
  condition({
    ...
    requires: ["actionable"],
    selector: (f) => stats(f).strategyAxisCount >= 3 && f.actionable === true,
  ...
  condition({
    ...
    requires: ["actionable"],
    selector: (f) => stats(f).strategyAxisCount >= 4 && f.actionable === true,
  ...
  condition({
    ...
    selector: (f) => {
      const s = stats(f);
      return s.strategyMatchCount >= 5 && s.strategyAxisCount >= 3 && f.actionable === true;
    },
